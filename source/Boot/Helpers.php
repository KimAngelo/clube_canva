 ...
 * @return string
 */
function image_glide(string $image, string $path, array $config)
{
    return url("/img/{$image}?" . http_build_query(array_merge($config, ['path' => $path])));
}
...
function status(string $status)
{
    switch ($status) {
        case "1":
            $status = "<span class=\"label label-success label-inline font-weight-bold mr-2\">Ativo</span>";
            break;
        case "2":
            $status = "<span class=\"label label-info label-inline font-weight-bold mr-2\">Reembolsada</span>";
            break;
        case "3":
            $status = "<span class=\"label label-danger label-inline font-weight-bold mr-2\">Cancelado</span>";
            break;
        case "4":
            $status = "<span class=\"label label-warning label-inline font-weight-bold mr-2\">Bloqueado</span>";
            break;
    }
 ...
 * @return string
 */
function balance_arts_label()
{
    $balance = \user()->artBalance();
    if ($balance <= 0) {
        return "label-danger";
    }
 ...
 * @return bool|string
 */
function generate_password($tamanho, $maiusculas, $minusculas, $numeros, $simbolos)
{
